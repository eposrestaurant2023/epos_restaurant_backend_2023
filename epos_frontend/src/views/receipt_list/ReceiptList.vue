<template>
  <PageLayout title="Receipt List" icon="mdi-note-outline" full>
    <div>
      <div class="flex mb-4">
        <div class="w-44 ">
          <v-text-field density="compact" variant="solo" label="Sale Number" single-line hide-details
            v-debounce="onSearchSale('name', 'like', $event)"></v-text-field>
        </div>
        <div class="w-44">
          <v-text-field label="Sale Number"  variant="solo" single-line hide-details
            density="compact" v-debounce="onSearchSale('customer', 'like', $event)"></v-text-field>
        </div>
        <div class="w-44">
          <v-text-field label="Customer name"  variant="solo" single-line hide-details
            density="compact"
            v-debounce="onSearchSale('customer_name', 'like', $event)"
            ></v-text-field>
        </div>
        <div class="w-44">
          <v-text-field label="Sale Number" v-model="current_date" variant="solo" single-line hide-details
      ...
      <v-table fixed-header hover>
        ...
        <tbody>
          <tr v-for="s in sales.data" :key="s.name">
            ...
            <td class="text-left"><v-btn variant="text" @click="onCustomer(s.customer)" class="pa-0">{{ s.customer }} -
                {{ s.customer_name }}</v-btn></td>
...
async function onDialog(name) {
  await openDialog(SaleDetail,{selected:name});
  alert(123)
