                        ...
                        <div class="flex justify-between">
                            <div> 
                                <div v-if="!cashierShiftReports.loading && cashierShiftReports.data?.length > 0 && activeReport.name == 'Cashier Shift'"> 
                                    <v-btn v-for="(r, index) in cashierShiftReports.data" :key="index" :color="activeReport.preview_report == r.name ? 'info' : 'default'" class="m-1" @click="onPrintFormat(r.name)">{{ r.title }}</v-btn>
                                </div>
                                <div v-else-if="workingDay?.length > 0 && activeReport.name == 'Working Day'"> 
                                    <v-btn v-for="(r, index) in workingDay" :key="index" class="m-1" :color="activeReport.preview_report == r.name ? 'info' : 'default'" @click="onPrintFormat(r.name)">{{ r.title }}</v-btn>
                                </div>
                            </div>
...
const printPreviewUrl = computed(()=>{
    return `${serverUrl}/printview?doctype=${activeReport.value.doc_type}&name=${activeReport.value.report_id}&format=${activeReport.value.preview_report}&no_letterhead=0&show_toolbar=0&letterhead=${activeReport.value.letterhead}&settings=%7B%7D&_lang=${activeReport.value.lang}`
})
...
function onPrintFormat(name){ 
    activeReport.value.preview_report = name;
    onRefresh()
...
function onWorkingDay(working_day){
    ...
    activeReport.value.print_report_name = working_day[0].print_report_name || working_day[0].name
}
