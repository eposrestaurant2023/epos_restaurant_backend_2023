<template>
    <div v-if="!current_working_day.loading">
        <ComButton icon-class="text-white" class="bg-red-600 text-gray-100" v-if="current_working_day.data"
            :title="$t('Close Working Day')" icon="mdi-calendar-clock" @click="onCloseWorkingDay" />
        <ComButton v-else :title="$t('Start Working Day')" icon="mdi-calendar-clock" @click="onStartWorkingDay" />
    </div>
    <div v-else
