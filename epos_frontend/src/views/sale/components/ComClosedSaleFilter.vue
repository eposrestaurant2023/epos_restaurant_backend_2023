<template>
    <div class="h-full">
        ...
        <div class="overflow-y-auto p-1">
            ...
            <v-select v-if="reportOption.show_cashier_shift" label="Cashier Shift" :items="filterResource.data?.cashier_shifts"
                variant="solo" v-model="filter.cashier_shift" item-title="title" item-value="name"></v-select>
            <v-select v-if="reportOption.show_outlet" label="Outlet" :items="filterResource.data?.outlets" variant="solo"
                v-model="filter.outlet" item-title="title" item-value="name"></v-select>
            <v-select v-if="reportOption.show_table_group" label="Table Group" :items="filterResource.data?.table_groups"
                variant="solo" v-model="filter.table_group" item-title="title" item-value="name"></v-select>

            <v-select v-if="reportOption.show_sale_type" label="Sale Type" :items="filterResource.data?.sale_types" variant="solo"
                v-model="filter.sale_type" item-title="title" item-value="name"></v-select>
            <v-select v-if="reportOption.show_payment_type" label="Payment Type" :items="paymentTypes" variant="solo"
                v-model="filter.payment_type" item-title="payment_method" item-value="payment_method"></v-select>
          
            <div class="flex justify-between -m-1">
                <div class="m-1 grow">
                    <v-select v-if="reportOption.show_order_by" label="Order by" :items="orderByOptions" variant="solo"
                    v-model="filter.order_by" item-title="title" item-value="name"></v-select>
                </div>
                <div class="m-1 flex-none">
                    <v-select v-if="reportOption.show_order_by" label="Order Type" :items="orderByType" variant="solo"
...
<script setup>
import { defineProps, defineEmits, ref, inject,computed } from '@/plugin';
